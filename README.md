...
Next.js only reads environment files from that folder. Use the template at
`web/.env.example` (or the root `.env.example`) to create `web/.env.local`
(or `web/.env`) and fill in your Firebase keys there;
